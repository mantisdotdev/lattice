...
//! operations from the surface that exists today (save, undo), undo everything,
//! and assert the user-visible state returns exactly to the seed snapshot.
//!
//! The equality domain is the one the gate names — working-tree bytes and the
//! checkpoint graph (lines and changesets do not exist yet) — explicitly
//! EXCLUDING the op-log, which grows on every operation including undo.
//!
...
use ltx_core::{Checkpoint, ChunkId, Repo};
...
fn snapshot(root: &Path, repo: &Repo) -> Snapshot {
    ...
    let checkpoints = repo
        ...
        .expect("reachable_checkpoints must not fail on a healthy repo");
    Snapshot { tree, checkpoints }
}
...
fn run_sequence(base: Option<&Path>, rng: &mut Rng, emitted: &mut BTreeMap<String, u64>) -> bool {
    ...
    // Apply a random batch. `save` does not touch the working tree (it
    // snapshots), so undo — pure head movement — restores the checkpoint graph
    // without any working-tree replay, which is the whole property here.
    let length = 1 + rng.below(12);
    let mut applied = 0u64;
    for _ in 0..length {
        // 0 => save, 1 => undo. Bias slightly toward save so there is usually
        // something to undo.
        if rng.below(3) == 0 {
