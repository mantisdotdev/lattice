...
def linearizability_violations(events: list[dict]) -> list[dict]:
    ...
    for i, a in enumerate(ordered):
        for b in ordered[i + 1:]:
            if b["start"] < a["end"]:
                break          # intervals overlap: any order is permitted
...
def main() -> int:
    ...
    try:
        ...
        violations = linearizability_violations(events)
        ...
        total = len(failures) + len(deadlocks) + len(violations) + (1 if corrupt else 0)
        return L.emit({
